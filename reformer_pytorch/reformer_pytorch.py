...
# carefully set for half precision to work
...
class LSHAttention(nn.Module, ABC):
    def __init__(self,
                 dropout=0.,
                 bucket_size=64,
                 n_hashes=8,
    ...
    def hash_vectors(self, n_buckets, vectors):
        ...
        random_rotations = torch.randn(rotations_shape, dtype=vectors.dtype, device=device).expand(batch_size, -1, -1,
                                                                                                   -1)

        dropped_vectors = self.dropout_for_hash(vectors)

        # dropped_vectors size [batch,seq_len,d_model]
        # random_rotations size [batch,d_model,n_hash,buckets//2]
        rotated_vectors = torch.einsum('bsd,bdni->bnsi', dropped_vectors, random_rotations)
        # rotated_vectors size [batch,n_hash,seq_len,buckets//2]
        ...
        if self._rehash_each_round:
            ...
            rotated_vectors = torch.squeeze(rotated_vectors, 0)
...
class FullQKAttention(nn.Module, ABC):
...
class ReformerLM(nn.Module, ABC):
    def __init__(self, num_tokens, dim, depth, max_seq_len, heads=8, dim_head=None, bucket_size=64, n_hashes=4,
                 ...
                 use_full_attn=False, full_attn_threshold=0, reverse_thres=0, num_mem_kv=0, one_value_head=False,
        ...
        self.reformer = Reformer(dim, depth, max_seq_len, heads=heads, dim_head=dim_head, bucket_size=bucket_size,
                                 ...
                                 full_attn_thres=full_attn_threshold, reverse_thres=reverse_thres,
